...
export class APIProfiles extends IAPI {
  search(
    ...
    return this.fetch<Paginated<ProfilePreviewDTO>>(
      "GET",
      Routes.Profiles.List,
      {
        query: {
          ...rest,
  ...
  getById(id: string): Promise<APIResponse<ProfileDTO>> {
    return this.fetch<ProfileDTO>(
      "GET",
      Routes.Profiles.ById.replace(":id", id),
    );
  }

  like(id: string): Promise<APIResponse<InteractionStateDTO>> {
    return this.fetch<InteractionStateDTO>(
      "POST",
      Routes.Profiles.Like.replace(":id", id),
    );
  }
